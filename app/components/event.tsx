...
const Carousel: React.FC<CarouselProps> = ({ onJoinEvent }) => {
  ...
  <div className="p-8 sm:p-12">
    <div className="grid md:grid-cols-2 gap-8 items-center">
      ...
      {/* Left Column - Event Details with animation */}
      <motion.div 
        className="space-y-6"
      ...
      >
        <div className="flex flex-wrap gap-3 mb-4">
          <motion.span 
            className="px-4 py-2 bg-gradient-to-r from-red-900 to-slate-800 text-white rounded-full text-sm font-semibold shadow-lg"
            whileHover={{ scale: 1.1 }}
          >
            {event["Event Type"]}
          </motion.span>
          <motion.span 
            className="px-4 py-2 bg-gradient-to-r from-slate-700 to-red-800 text-white rounded-full text-sm font-semibold shadow-lg"
            whileHover={{ scale: 1.1 }}
          >
            {event.Location}
          </motion.span>
        </div>
        {/* Add remaining event details here */}
      </motion.div>

      {/* Right Column - Add image or extra info here if needed */}
      <div>
...
>
  ...
  >
    <motion.div 
      className="w-32 h-32 mx-auto bg-gradient-to-br from-red-700 to-slate-800 rounded-full flex items-center justify-center mb-6 shadow-2xl"
      whileHover={{ scale: 1.1 }}
    >
      <motion.div
        animate={{ rotate: 360 }}
...
>
  <motion.h1 
    className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 text-gray-900"
    initial={{ opacity: 0, y: 30 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.8, delay: 0.2 }}
  >
  ...
  <motion.p 
    className="text-gray-600 text-lg sm:text-xl max-w-3xl mx-auto leading-relaxed mb-8"
    initial={{ opacity: 0, y: 30 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.8, delay: 0.4 }}
  >
            ...
            {filteredEvents.slice(0, eventsToShow).map((event, index) => (
              ...
              >
                ...
                <div className={`relative bg-white rounded-2xl border-2 ${getEventTypeBgColor(event["Event Type"]).split(' ')[1]} shadow-lg hover:shadow-2xl transition-shadow duration-300 overflow-hidden`}>
                  ...
                  <div className="p-6">                {/* Card Glow Effect */}
<motion.div 
